...
fun <T : Observable> SqlContext.createOrUpdateTable(key: String, clazz: KClass<T>) : Boolean{
    ...
    return if(checkIfTableExists(key)){
        ...
        val props = clazz.memberProperties
                ...
                .map {
                    val notNull = " " + if(it.returnType.isMarkedNullable) "" else "NOT NULL"
                    val type = it.getter.returnType.javaType
                    val datatype = if(it.javaField == pkProp.javaField /* because uuid would be Observable.uuid and Test.uuid */
                            && type == String::class.java){
                        "VARCHAR(255)"
                    }else{
                        typeMap[type]!!
                    }
                    it.name to (datatype) + notNull
                }
                .toMap().toMutableMap()
...
fun getSqlFieldName(props : List<KProperty1<*, *>>) = props.map { it.name }.joinToString("_")
